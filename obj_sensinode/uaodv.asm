...
; This file was generated Sat Jan 27 13:27:29 2018
;--------------------------------------------------------
	.module uaodv
...
;------------------------------------------------------------
;host                      Allocated to registers r2 r3 r4 
...
;	../../../core/net/uaodv.c:72: if(route != NULL)
	...
	sjmp	00102$
00106$:
;	../../../core/net/uaodv.c:73: return htonl(route->hseqno);
	mov	a,#0x0B
	add	a,r2
	mov	r2,a
	clr	a
...
;------------------------------------------------------------
;id                        Allocated to stack - offset -5
;orig                      Allocated to stack - offset 1
;n                         Allocated to registers r2 r3 
;sloc0                     Allocated to stack - offset 4
;------------------------------------------------------------
;	../../../core/net/uaodv.c:89: fwc_lookup(const uip_ipaddr_t *orig, const u32_t *id)
;	-----------------------------------------
;	 function fwc_lookup
;	-----------------------------------------
_fwc_lookup:
...
;sloc0                     Allocated to stack - offset 4
;------------------------------------------------------------
;	../../../core/net/uaodv.c:96: fwc_add(const uip_ipaddr_t *orig, const u32_t *id)
;	-----------------------------------------
;	 function fwc_add
;	-----------------------------------------
_fwc_add:
...
;rm                        Allocated to stack - offset 4
...
;	../../../core/net/uaodv.c:195: rm->hop_count = 0;
	...
	clr	a
	lcall	__gptrput
;	../../../core/net/uaodv.c:196: rm->rreq_id = htonl(rreq_id++);
	mov	a,_bp
	add	a,#0x04
	mov	r0,a
	mov	a,_bp
...
;	../../../core/net/uaodv.c:199: my_hseqno++;			/* Always */
	...
	addc	a,#0x00
	movx	@dptr,a
;	../../../core/net/uaodv.c:200: rm->orig_seqno = htonl(my_hseqno);
	mov	a,_bp
	add	a,#0x04
	mov	r0,a
	mov	a,_bp
...
;orig                      Allocated to stack - offset -8
;seqno                     Allocated to stack - offset -11
;hop_count                 Allocated to stack - offset -13
...
;	../../../core/net/uaodv.c:222: uip_ipaddr_copy(&rm->dest_addr, dest);
	...
	add	a,#0xfb
	mov	sp,a
;	../../../core/net/uaodv.c:223: rm->lifetime = HTONL(MY_ROUTE_TIMEOUT);
	mov	a,_bp
	add	a,#0x04
	mov	r0,a
	mov	a,#0x10
...
;------------------------------------------------------------
;seqno                     Allocated to stack - offset -5
;addr                      Allocated to stack - offset 1
;rm                        Allocated to stack - offset 4
;sloc0                     Allocated to stack - offset 7
;------------------------------------------------------------
;	../../../core/net/uaodv.c:228: send_rerr(uip_ipaddr_t *addr, u32_t *seqno)
;	-----------------------------------------
;	 function send_rerr
;	-----------------------------------------
_send_rerr:
...
;------------------------------------------------------------
;rm                        Allocated to stack - offset 1
;dest_addr                 Allocated to stack - offset 4
;orig_addr                 Allocated to stack - offset 8
;rt                        Allocated to stack - offset 12
;fw                        Allocated to stack - offset 15
;net_seqno                 Allocated to stack - offset 18
;net_seqno                 Allocated to stack - offset 22
;len                       Allocated to registers 
;sloc0                     Allocated to stack - offset 26
;sloc1                     Allocated to stack - offset 29
;sloc2                     Allocated to stack - offset 32
;sloc3                     Allocated to stack - offset 36
;------------------------------------------------------------
...
;	../../../core/net/uaodv.c:309: rt = uaodv_rt_lookup(&rm->orig_addr);
	...
	inc	r0
	mov	@r0,ar4
;	../../../core/net/uaodv.c:311: || (SCMP32(ntohl(rm->orig_seqno), rt->hseqno) > 0) /* New route. */
	mov	a,_bp
	add	a,#0x0c
	mov	r0,a
	cjne	@r0,#0x00,00168$
...
00168$:
	...
	ljmp	00106$
00169$:
;	../../../core/net/uaodv.c:312: || (SCMP32(ntohl(rm->orig_seqno), rt->hseqno) == 0
	mov	a,_bp
	add	a,#0x1a
	mov	r0,a
	mov	dpl,@r0
...
;	../../../core/net/uaodv.c:325: if(!(rm->flags & UAODV_RREQ_UNKSEQNO)
	...
	ljmp	00119$
00177$:
;	../../../core/net/uaodv.c:327: && SCMP32(fw->hseqno, ntohl(rm->dest_seqno)) <= 0) {
	mov	a,_bp
	add	a,#0x0f
	mov	r0,a
	cjne	@r0,#0x00,00178$
...
;	../../../core/net/uaodv.c:337: uip_ipaddr_copy(&orig_addr, &rm->orig_addr);
	...
	add	a,#0xfb
	mov	sp,a
;	../../../core/net/uaodv.c:338: net_seqno = htonl(fw->hseqno);
	mov	a,_bp
	add	a,#0x0f
	mov	r0,a
	mov	a,#0x0B
...
;	../../../core/net/uaodv.c:349: if(!(rm->flags & UAODV_RREQ_UNKSEQNO)
	...
	ljmp	00129$
00185$:
;	../../../core/net/uaodv.c:350: && SCMP32(my_hseqno, ntohl(rm->dest_seqno)) < 0) {
	mov	r0,_bp
	inc	r0
	mov	a,_bp
	add	a,#0x24
	...
	mov	r3,a
	jnb	acc.7,00129$
;	../../../core/net/uaodv.c:352: my_hseqno = ntohl(rm->dest_seqno) + 1;
	mov	a,_bp
	add	a,#0x24
	mov	r0,a
	mov	dpl,@r0
	...
	movx	@dptr,a
00129$:
;	../../../core/net/uaodv.c:354: net_seqno = htonl(my_hseqno);
	mov	dptr,#_my_hseqno
	movx	a,@dptr
	mov	r2,a
	inc	dptr
...
;------------------------------------------------------------
;rm                        Allocated to stack - offset 1
;rt                        Allocated to stack - offset 4
;ack                       Allocated to registers r2 r3 r4 
;sloc0                     Allocated to stack - offset 7
;------------------------------------------------------------
...
;	../../../core/net/uaodv.c:413: rt = uaodv_rt_lookup(&rm->dest_addr);
	...
	inc	r0
	mov	@r0,ar4
;	../../../core/net/uaodv.c:416: if(rt == NULL || (SCMP32(ntohl(rm->dest_seqno), rt->hseqno) > 0)) {
	mov	a,_bp
	add	a,#0x04
	mov	r0,a
	cjne	@r0,#0x00,00146$
...
00137$:
	...
	ljmp	00110$
00138$:
;	../../../core/net/uaodv.c:480: || SCMP32(rt->hseqno, ntohl(rm->unreach[0].seqno)) <= 0) {
	push	ar5
	push	ar6
	push	ar7
	mov	a,_bp
...
;	../../../core/net/uaodv.c:483: rm->flags &= ~UAODV_RERR_UNKNOWN;
	...
	mov	a,r2
	lcall	__gptrput
;	../../../core/net/uaodv.c:484: rm->unreach[0].seqno = htonl(rt->hseqno);
	mov	r0,_bp
	inc	r0
	mov	a,#0x04
	add	a,@r0
...
;	../../../core/net/uaodv.c:531: rt->is_bad = 1;
	...
	mov	a,#0x01
	lcall	__gptrput
;	../../../core/net/uaodv.c:532: bad_seqno = htonl(rt->hseqno);
	mov	a,#0x0B
	add	a,r5
	mov	r5,a
	clr	a
...
;	../../../core/net/uaodv.c:577: printf("uaodv_process starting %lu\n", (unsigned long) my_hseqno);
	...
	pop	ar3
	pop	ar2
;	../../../core/net/uaodv.c:579: bcastconn = udp_broadcast_new(HTONS(UAODV_UDPPORT), NULL);
	push	ar3
	push	ar4
	clr	a
	push	acc
	...
	mov	a,r2
	movx	@dptr,a
;	../../../core/net/uaodv.c:580: unicastconn = udp_broadcast_new(HTONS(UAODV_UDPPORT), NULL);
	clr	a
	push	acc
	push	acc
	push	acc
